...
export const syncAllergies = async ({ original, updated, friendId = null }) => {
  ...
  await Promise.all([
    ...toDelete.map((a) => deleteAllergy(a.id)),
    ...toAdd.map((a) =>
      addAllergy({ name: a.name, severity: a.severity, userCustom: a.userCustom, friendId })
    ),
    ...toUpdate.map((a) => updateAllergySeverity(a.id, a.severity)),
  ]);
};
