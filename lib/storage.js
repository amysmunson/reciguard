...
export const KEYS = {
  ...
  allergyOverviewFilter: (userId) => `allergyOverview.filter:${userId}`,
...
// Wipes every per-user preference key above. Called on sign-out/account
// deletion. Every entry in KEYS is a (userId) => key function, so this
// stays complete automatically as new KEYS.* entries are added, rather than
// needing a second hand-maintained list.
export const clearUserPrefs = async (userId) => {
  if (!userId) return;
  await Promise.all(Object.values(KEYS).map((keyFn) => removeKey(keyFn(userId))));
};
