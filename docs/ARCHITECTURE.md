...
2. **Groups** — composite entries (*Tree nuts*, *Dairy*, etc., 35 total).
   Tapping a group adds/removes every member preset at once; half-checked
   groups render a half-circle icon.
...
- **Group checkboxes track which group was tapped, not raw preset
  ...
  *explicitly* tapped), not from re-deriving it off however many of its
  ...
  Unchecking a group also actively clears any *other* selected group that
  shares members with it (`toggleGroup` scans `selectedGroupIds` for
  overlap on removal) — without this, e.g. unchecking "All shellfish"
  after also checking "Crustacean shellfish" and "Molluscs" would leave
  those two rendering a stale half-circle instead of going back to a
  plain unchecked box, since their own members had just been removed out
  from under them.
